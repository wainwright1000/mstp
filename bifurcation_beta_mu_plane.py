"""
Two-parameter bifurcation diagram in the (beta, mu) plane.

Colours indicate the number of equilibria (fixed points) of f_2(x)
for each combination of beta and mu, with rho, W1, W2 held fixed.
"""
...
def generate_diagram(
    output_path='figures/bifurcation_beta_mu_plane.png',
    beta_range=(0, 20),
    mu_range=(0, 1),
    n_points=250,
    n_guess=100,
):
    """Generate a 2-D bifurcation diagram in the (beta, mu) plane."""
    ...
    print(f"Scanning {total} grid points (beta x mu) ...")
    ...
    ax.set_title(
        r'Regime diagram in the $(\beta, \mu)$ plane' + '\n'
        r'$\rho={rho}, W_1={W1}, W_2={W2}$'.format(**params),
        fontsize=14,
    )
