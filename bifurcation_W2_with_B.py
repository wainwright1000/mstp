...
Parameters: μ=0.4, β=14, ρ=1, W₁=-0.6
"""
...
def generate_diagram(output_path='figures/bifurcation_W2_with_B.png', mu_value=0.4):
    ...
    W2_critical = scale_b_to_axis(normalized_critical, param_range)
    ax.axvline(x=W2_critical, color='grey', linestyle='--', linewidth=1, 
               alpha=0.7, zorder=0)
    
    # Calculate and plot B(x*) at equilibrium points
    B_x_vals = []
    B_y_vals = []
    for W2_val, x_val in zip(stable_x + unstable_x, stable_y + unstable_y):
    ...
    for y_val in intersection_ys:
        def g_at_y(W2):
            return abs(fixed_point_equation(y_val, params['mu'], params['beta'], 
                                            params['rho'], params['W1'], W2))
        
        result = minimize_scalar(g_at_y, bounds=(W2_critical, 2.0), method='bounded')
        best_W2 = result.x
        best_g = result.fun
        
        if best_g < 0.01:
            new_turning_x.append(best_W2)
            new_turning_y.append(y_val)
            ax.plot([W2_critical, best_W2], [y_val, y_val],
                   color='grey', linestyle='--', linewidth=1, alpha=0.7, zorder=0)
            ax.plot(W2_critical, y_val, 'ko', markersize=7, markerfacecolor='none',
    ...
    ax.set_xlabel(r'$W_2$', fontsize=13)
    ax.set_ylabel('Equilibria', fontsize=13)
    ax.set_title(r'Bifurcation diagram of $f_2(x)$ varying $W_2$', fontsize=16)
