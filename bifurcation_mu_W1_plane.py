"""
Two-parameter bifurcation diagram in the (mu, W1) plane.

Colours indicate the number of equilibria (fixed points) of f_2(x)
for each combination of mu and W1, with beta, rho, W2 held fixed.
"""
...
def generate_diagram(
    output_path='figures/bifurcation_mu_W1_plane.png',
    mu_range=(0, 1),
    W1_range=(-1.5, 2),
    n_points=250,
    n_guess=100,
    ...
    print(f"Scanning {n_points*n_points} grid points (mu x W1) ...")
    for i, mu in enumerate(mu_vals):
        for j, W1 in enumerate(W1_vals):
            fps = find_fixed_points(
                mu=mu, beta=params['beta'], rho=params['rho'],
                W1=W1, W2=params['W2'], n_guess=n_guess,
            )
            n_fp[j, i] = len(fps)
    ...
    mesh = ax.pcolormesh(mu_vals, W1_vals, n_fp, shading='auto',
                         cmap=cmap, vmin=1, vmax=3)
    ax.contour(mu_vals, W1_vals, n_fp, levels=[2.0], colors='black',
               linewidths=1.5, linestyles='--')
    ax.plot(DEFAULT_PARAMS['mu'], DEFAULT_PARAMS['W1'], 'ko',
            markersize=8, markerfacecolor='white', markeredgewidth=2,
            label='Baseline $(\\mu, W_1)$', zorder=5)
    ...
    ax.set_title(
        r'Regime diagram in the $(\mu, W_1)$ plane' + '\n'
        r'$\beta={beta}, \rho={rho}, W_2={W2}$'.format(**params),
        fontsize=14,
    )
    ax.set_xlim(mu_range)
    ax.set_ylim(W1_range)
