...
def generate_diagram(output_path='figures/bifurcation_W2.png', mu_value=0.4):
    ...
    all_W2_vals = stable_x + unstable_x
    all_x_vals = stable_y + unstable_y
    
    threshold_points = []
    for i, (W2_val, x_val) in enumerate(zip(all_W2_vals, all_x_vals)):
        B_val = calculate_B(x_val, params['mu'], params['beta'], params['rho'],
                           params['W1'], W2_val)
        B_crit = 1 / (2 * params['beta'] * params['rho'])
        diff = B_val - B_crit
        if abs(diff) < 0.1:
            threshold_points.append((x_val, W2_val, diff))
    
    threshold_points = sorted(set(threshold_points))
    
    intersection_ys = []
    for i in range(len(threshold_points) - 1):
        y1, W2_1, diff1 = threshold_points[i]
        y2, W2_2, diff2 = threshold_points[i + 1]
        if abs(y2 - y1) > 0.2 or abs(W2_2 - W2_1) > 0.1:
            continue
        if diff1 * diff2 < 0:
